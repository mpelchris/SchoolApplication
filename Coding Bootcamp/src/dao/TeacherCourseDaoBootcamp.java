 ...
 * @author christy
 */
public class TeacherCourseDaoBootcamp {
    
    public List <User> getAllTeachers(Course course){
      List <User> list=new ArrayList<>();
      Connection con= DBschoolutils.getconnection();
      String sql="SELECT * FROM( users u inner join user_course uc ON u.userid=uc.userid AND u.roleid=2)  INNER JOIN courses c ON c.courseid=uc.courseid AND c.courseid=?;;";
    ...
    public List <Course>  getCoursesByTeachersId(int id){
        ...
        Connection con= DBschoolutils.getconnection();
        String sql="SELECT c.courseid,title,stream,startDate,endDate FROM( users u inner join user_course uc ON u.userid=uc.userid AND u.userid=?) INNER JOIN courses c ON c.courseid=uc.courseid ;	";
        PreparedStatement pst=null;
          try {
               pst=con.prepareStatement(sql);
              pst.setInt(1,id);
    ...
    public boolean insertTeacherIntoACourse(Course course,User user){
        Connection con= DBschoolutils.getconnection();
    ...
    public boolean deleteTeacherFromACourse(Course course,User user){
     Connection con= DBschoolutils.getconnection();
